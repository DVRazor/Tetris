var Render = function(glassWidth, glassHeight, lineWidth){
	...
	function onComplete(){
		...
		text = new createjs.Text("test", "20px Arial", 'red');
		text.x = 100;
		text.y = 50;
   ...
   function tick(){   		
	...
	scope.addFigure = function(figure){
		if(tween) tween.kill();
		// figureContainer.y = (figure.position.y - figure.states[figure.phase].center[0]) * SQUARE_SIZE;
	...
	function setFigure(figure){
		...
		// figureContainer.y = (figure.position.y - centerY) * SQUARE_SIZE;

		var newY = (figure.position.y) * SQUARE_SIZE;

		// text.text = figureContainer.y;
		// figureContainer.y = newY;
		if(tween) tween.kill();
		// if(newY - figureContainer.y > SQUARE_SIZE) tween = TweenLite.to(figureContainer, 0.8, {y : newY, ease: Power4.easeInOut, immediateRender: true});
		...
		console.log(figure.position.y, newY);
